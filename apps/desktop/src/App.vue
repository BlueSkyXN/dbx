    ...
    <TooltipProvider :delay-duration="300">
      <div class="h-screen w-screen max-w-full min-w-[760px] min-h-[600px] flex flex-col bg-background text-foreground overflow-hidden" :class="{ 'dbx-desktop-window-frame': drawDesktopWindowFrame }" :style="appUiFontFamilyStyle">
            ...
            <div class="h-full flex flex-col min-w-0">
              ...
              <div v-if="activeTab" v-show="!driverStoreActive && !settingsStore.settingsPageActive" class="flex flex-col flex-1 min-h-0">
                <EditorToolbar
                  ...
                  @update:explain-mode="(m) => (explainMode = m)"
                  @update:block-dangerous-redis-commands="(v) => (blockDangerousRedisCommands = v)"
                ...
                <KeepAlive :max="4">
                  <ContentArea
                    ...
                    @cancel="cancelActiveExecution()"
                    @explain="tryExplain()"
                    @editor-update="(tabId, v) => queryStore.updateSql(tabId, v)"
                    @editor-selection-change="(v) => (selectedSql = v)"
                    @editor-cursor-change="(p) => (cursorPos = p)"
                    @editor-viewport-change="(tabId, viewport) => queryStore.updateEditorViewport(tabId, viewport)"
                    @editor-selection-state-change="(tabId, selection) => queryStore.updateEditorSelection(tabId, selection)"
                    @format-error="toast(t('toolbar.formatSqlFailed'))"
                    @save-sql="void openSaveSqlDialog()"
                    @reload="(sql, searchText, whereInput, orderBy, limit, offset) => onReloadData(sql, searchText, whereInput, orderBy, limit, offset)"
                    @paginate="onPaginate"
                    @sort="onSort"
        ...
        <AppDialogs
          ...
          @connect-started="(name) => toast(t('connection.connecting', { name }), 30000)"
          @connect-succeeded="(name) => toast(t('connection.connectSuccess', { name }), 2000)"
          @connect-failed="
            (msg: string) =>
              toast(
                t('connection.connectFailed', {
