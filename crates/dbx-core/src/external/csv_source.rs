...
impl CsvSource {
    pub fn new(file_path: PathBuf, mut config: CsvExternalConfig) -> Self {
        if config.delimiter == ','
            && file_path.extension().and_then(|value| value.to_str()).is_some_and(|ext| ext.eq_ignore_ascii_case("tsv"))
        {
            config.delimiter = '\t';
        }
...
fn csv_char_byte(ch: char, label: &str) -> Result<u8, String> {
    if ch.is_ascii() {
        Ok(ch as u8)
    } else {
        Err(format!("CSV {label} must be a single-byte ASCII character"))
    }
}
...
mod tests {
    ...
    fn temp_file(extension: &str, contents: &str) -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!("dbx_csv_source_{nanos}.{extension}"));
        std::fs::write(&path, contents).unwrap();
        path
    }

    #[test]
    fn infers_tsv_delimiter_from_extension_when_config_uses_default_delimiter() {
        let path = temp_file("tsv", "id\tname\n1\tAda\n");
        let source = CsvSource::new(path.clone(), CsvExternalConfig::default());

        let (headers, rows) = source.read_csv().unwrap();
