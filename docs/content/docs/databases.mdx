...
| External tabular sources       | CSV, XLSX, Feishu Sheets, Feishu Bitable                                                                                                                         | Query local files and Feishu cloud tables through an in-memory DuckDB cache                    |
...
CSV/XLSX files and Feishu cloud tables are loaded into an in-memory DuckDB cache before querying. CSV/XLSX connections are local snapshots. Feishu Sheets and Bitable connections can be refreshed manually, or configured to refresh before each `SELECT`.

Feishu Sheets supports sheet discovery, range reads, append writes, and explicit range overwrite through the external write API. Feishu Bitable supports table and field discovery, record search, append, update, and delete operations. See [Cloud Table Sources](/en/docs/cloud-sources) for connection fields and sync behavior.
