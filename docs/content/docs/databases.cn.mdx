---
title: 数据库支持
description: 通过 Rust 原生驱动支持 25+ 种数据库，无 JDBC 依赖。
---

<Callout type="info">
Rust 原生驱动，不依赖 JDBC。MySQL / PostgreSQL 兼容数据库开箱即用。
</Callout>

## 完整支持

| 数据库 | 驱动 | 协议 |
|---|---|---|
| MySQL | sqlx | MySQL |
| PostgreSQL | sqlx | PostgreSQL |
| SQLite | sqlx | 嵌入式 |
| Redis | redis-rs | RESP |
| MongoDB | mongodb | MongoDB Wire |
| DuckDB | duckdb-rs | 嵌入式 |
| ClickHouse | HTTP client | HTTP |
| SQL Server | tiberius | TDS |
| Oracle | oracle-rs | OCI |
| Elasticsearch | HTTP client | REST |
| DM（达梦） | odbc-api | ODBC |
| GaussDB | rust-gaussdb | GaussDB |
| 飞书电子表格 | Feishu OpenAPI + DuckDB cache | HTTPS |
| 飞书多维表格 | Feishu OpenAPI + DuckDB cache | HTTPS |

## MySQL 兼容

以下数据库使用 MySQL 协议，通过 MySQL 驱动连接，开箱即用：

| 数据库 | 默认端口 |
|---|---|
| MariaDB | 3306 |
| TiDB | 4000 |
| OceanBase | 2881 |
| GoldenDB | 3306 |
| Doris | 9030 |
| SelectDB | 9030 |
| StarRocks | 9030 |
| TDengine | 6030 |

<Callout type="info">
OceanBase 支持自动检测 Oracle 模式 — 当连接的 OceanBase 实例运行在 Oracle 模式下时，DBX 会自动切换为 Oracle 风格的 Schema 查询。
</Callout>

以下数据库使用 PostgreSQL 协议，通过 PostgreSQL 驱动连接：

| 数据库 | 默认端口 |
|---|---|
| openGauss | 5432 |
| KingBase（人大金仓） | 54321 |
| Vastbase | 5432 |
| Redshift | 5439 |
| CockroachDB | 26257 |

## ODBC 数据库

DM（达梦）通过 ODBC 连接，需要在系统上安装对应的 ODBC 驱动。

## 外部表格数据源

CSV/XLSX 文件和飞书云文档表格会先加载到内存 DuckDB 缓存中再查询。CSV/XLSX 是本地快照；飞书电子表格和多维表格可以手动刷新，也可以配置为每次 `SELECT` 前刷新。

飞书电子表格支持发现工作表、读取范围、追加写入，以及通过外部写入 API 覆盖指定范围。飞书多维表格支持发现数据表和字段、查询记录、追加、更新和删除记录。连接字段和同步行为见[云表格数据源](/cn/docs/cloud-sources)。

### DM（达梦）

达梦 DM8 通过 ODBC 驱动连接。默认端口：**5236**，默认用户：**SYSDBA**。

<Tabs groupId="dm-platform" items={['Linux', 'Windows', 'macOS']}>
  <Tab value="Linux">
    ```bash
    # 安装 unixODBC
    sudo apt install unixodbc

    # 安装达梦数据库（安装包含 ODBC 驱动）
    # 下载地址：https://eco.dameng.com/download/

    # 在 /etc/odbcinst.ini 中注册驱动
    sudo tee -a /etc/odbcinst.ini << 'EOF'
    [DM8 ODBC DRIVER]
    Description = DM8 ODBC Driver
    Driver = /opt/dmdbms/bin/libdodbc.so
    EOF
    ```
  </Tab>
  <Tab value="Windows">
    达梦安装程序会自动注册 ODBC 驱动，无需手动配置。
  </Tab>
  <Tab value="macOS">
    达梦暂未提供 macOS 原生 ODBC 驱动。开发/测试建议使用 Linux 环境连接。
  </Tab>
</Tabs>
