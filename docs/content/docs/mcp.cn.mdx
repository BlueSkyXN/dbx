...
DBX MCP Server 也可以直接调用飞书 OpenAPI。这些工具不依赖 DBX 连接，可以通过环境变量配置认证，也可以在单次调用中传入认证字段。

| 工具 | 说明 |
|---|---|
| `dbx_feishu_get_tenant_access_token` | 使用自建应用凭证获取飞书 `tenant_access_token` |
| `dbx_feishu_sheets_info` | 获取电子表格元数据和工作表列表 |
...
电子表格工具支持传 `spreadsheet_token` 或 `/sheets/` URL；多维表格工具支持传 `base_token` 或 `/base/` URL。
